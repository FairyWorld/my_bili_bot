...
def main_handler():
    ...
    #创建B站专栏
    article = Article(cookies, "每日动漫美图") #创建B站专栏草稿,并设置标题
    ...
    for i in range(num):
        try:
            ...
            # https://api.gupan.site/api/img/acg.php
            # https://api.yuzhitu.cn//sjbz/api.php?lx=dongman
            res = session.get('https://api.ixiaowai.cn/api/api.php', headers=headers) #这里得到P站图片
