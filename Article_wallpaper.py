...
def main_handler():
    ...
    num = 10 #只爬取18张图,可以调大，如果中间网络异常会丢失几张图，最终数量可能达不到

    #创建B站专栏
    article = Article(cookies, "每日4k壁纸") #创建B站专栏草稿,并设置标题
    ...
    for i in range(num):
        try:
            res = session.get('https://api.ixiaowai.cn/gqapi/gqapi.php', headers=headers) #这里得到P站图片
